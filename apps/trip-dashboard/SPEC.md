...
メールにプレーンテキストとHTMLの両方がある場合、以下の順で本文を選択する。

1. プレーンテキストに予約番号、チェックイン、チェックアウト、フライト詳細、便情報のいずれかが含まれる場合はプレーンテキストを使用する
2. 上記に該当しない場合はHTMLをテキスト化して使用する
3. HTMLがない場合はプレーンテキストを使用する
