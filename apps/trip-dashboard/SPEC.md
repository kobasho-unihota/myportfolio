...
`Content-Transfer-Encoding` が quoted-printable の場合は復号してから、メールパートの `charset` を参照して文字コードを変換する。`TextDecoder` が対象文字コードを提供しないブラウザでは `FileReader.readAsText` を使用する。HTML内のリンクは、リンク文字列とURLを保持したテキスト形式へ変換する。
